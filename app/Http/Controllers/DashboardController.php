...
class DashboardController extends Controller
{
    ...
    public function profileUpdateIndex(Request $request)
    {
        $response = [];
        $response['country_codes'] = json_decode(File::get(storage_path() . "/json/country-codes.json"),true);
        $response['nationalities'] = json_decode(File::get(storage_path() . "/json/nationalities.json"),true);
        $ud = UserDetails::select('avatar_url')->where('users_id', $request->session()->get('user')->id)->get();
        $response['avatar'] = $ud[0]->avatar_url;
        return view('profile-update', $response);
    }

    public function incompleteIndex(Request $request)
    {
    	$response = [];
    	$response['country_codes'] = json_decode(File::get(storage_path() . "/json/country-codes.json"),true);
    	$response['nationalities'] = json_decode(File::get(storage_path() . "/json/nationalities.json"),true);

    	return view('incomplete-profile', $response);
    }

    public function submitIncompleteProfile(Request $request)
    {
        $draft = (object)[];
        ...
        if($request->hasFile('avatar_img')) {
            ...
            $request->logo->move(public_path('company_logos'), $avatar_name);
            $logoUrl = asset('/media/avatars') . '/' . $logo_name;
        }
